...
function PlaceList({ places, emptyMessage }: { places: NearbyPlace[]; emptyMessage: string }) {
  if (places.length === 0) {
  ...
  return (
    <ul className="space-y-3">
      {places.map((place) => (
        <li key={place.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
...
export default function NearbyPlacesPanel({ tripId, hasLocations }: NearbyPlacesPanelProps) {
  ...
  if (!hasLocations) {
    return (
      <p className="text-gray-500">
        Add at least one trip location to search for nearby mosques and Halal restaurants.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {loading && <p className="text-gray-500">Searching within 5 km of your trip area...</p>}
      {error && (
        <p className="text-red-600">
          {error}. Ensure Google Places API is enabled and `GOOGLE_MAPS_API_KEY` is set on the API.
        </p>
      )}
      ...
      {!loading && !error && (
        <Tabs defaultValue="mosques">
          ...
          <TabsContent value="mosques" className="mt-4">
            <PlaceList places={mosques} emptyMessage="No mosques found nearby." />
          </TabsContent>
          <TabsContent value="halal" className="mt-4">
            <PlaceList places={halal} emptyMessage="No Halal restaurants found nearby." />
          </TabsContent>
        </Tabs>
      )}
    </div>
