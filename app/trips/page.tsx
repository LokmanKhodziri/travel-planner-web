...
export default async function TripsPage() {
  ...
  const upcomingTrips = sortedTrips.filter((t) => new Date(t.startDate) >= today);
    ...
    <div className="space-y-8 container mx-auto px-4 py-10">
      ...
      <section>
        <Card className="bg-gray-50 border-gray-200">
          <CardHeader>
            <CardTitle className="text-2xl">Welcome back, {session.name ?? "there"}!</CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <Card className="flex flex-col items-center justify-center text-center py-12">
            <CardContent>
              ...
              <p className="text-gray-500 mb-6 max-w-sm">
                It looks like you haven&apos;t planned any trips. Click the button below to create your first one.
              </p>
              <Link href="/trips/new">
              ...
              <Link href={`/trips/${trip.id}`} key={trip.id}>
                <Card className="h-full flex flex-col hover:border-blue-500 transition-colors duration-200">
                  ...
                  <CardContent className="flex-grow flex flex-col">
                    <p className="text-gray-600 line-clamp-2 flex-grow">{trip.description}</p>
